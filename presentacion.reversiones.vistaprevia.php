...
$sqlRevExistente = "SELECT * FROM intepresentaciondetalle WHERE idpresentacion = $idPresentacion and nrocominterno = $idFactura";
$resRevExistente = mysql_query($sqlRevExistente);
$canRevExistente = mysql_num_rows($resRevExistente);
if ($canRevExistente == 0) {
    ...
    if ($canBusquedaFacturaAnterior != 0) {	
        ...
        $cuit = $rowBusquedaFacturaAnterior['cuit'];
        $sqlNombre = "SELECT nombre FROM madera.prestadores WHERE cuit = '$cuit'";
        $resNombre = mysql_query($sqlNombre);
        $canNombre = mysql_num_rows($resNombre);
        if ($canNombre != 0) {
            $rowNombre = mysql_fetch_assoc($resNombre);	
    	    $arrayReversion = array();
        	$index = 0;
        	$arrayReversion[$index]['datos'] = $rowBusquedaFacturaAnterior;
        	$arrayReversion[$index]['tipo'] = 'DB';
        	$arrayReversion[$index]['nombre'] = $rowNombre['nombre'];
        	//SOLO DB
        	if ($tipo == 1) {
        		$posibleReversion = 1;
        	} else {
	...
	<div align="center">
	 	...
	 	<?php if ($posibleReversion == 1) { ?>
	 		 ...
	 		 <form action="presentacion.reversiones.guardar.php?id=<?php echo $idPresentacion ?>" method="post" onSubmit="return validar()">
	 			<?php $revSerializado = serialize($arrayReversion);
	 			      $revSerializado = urlencode($revSerializado); ?>
				   <input name="reversion" type="hidden" value="<?php echo $revSerializado ?>"/>
				   <input name="tiporev" type="hidden" value="<?php echo $tipo ?>"/>
	 			<p><input type="submit" value="Guardar Reversion" style="margin-top: 15px"></input></p>
	 		</form>
	 	<?php } else { ?>
	 			<h3 style="color: red">Con la informacion cargada no se puede realizar la reversion</br>Revise la informacion cargada</h3>
	 	<?php } ?>
	 </div>
</body>
</html>
