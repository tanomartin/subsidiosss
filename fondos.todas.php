...
$sqlPresentacion = "SELECT 
						...
						DATE_FORMAT(intefondos.fechapresentacion, '%d-%m-%Y') as fechapresentacionfondo,
						DATE_FORMAT(intefondos.fechacancelacion, '%d-%m-%Y') as fechacancelacionfondo
					...
					LEFT JOIN intefondos on p.id = intefondos.idpresentacion
					WHERE p.fechacierrepagos is not null
					ORDER BY intefondos.id DESC";
						...
						<?php $estado = "SIN</br>PRESENTAR";
							  ...
							  if ($rowPresentacion['idfondo'] != NULL) { 
								  if ($rowPresentacion['fechafinalizacionfondo'] == NULL) {
						   	 	  ...
						   	 	  if ($rowPresentacion['fechacancelacionfondo'] != NULL) {
						   	 	  	$estado = "CANCELADA</br>(".$rowPresentacion['fechacancelacionfondo'].")";
						   	 	  	$color = 'style="color: red"';
						   	 	  }
							  } ?>
