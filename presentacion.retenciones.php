...
<body bgcolor="#CCCCCC">
	<div align="center">
		...
		<form action="presentacion.retenciones.guardar.php" onSubmit="return validar(this)"  method="post">
					...
					<?php } else {
							...
							if ($rowFactura['nopagar'] == 1) {
					 		...
					 		<tr>
					 			...
					 			<?php $checkquitar = "";
					 				  if ($rowFactura['nopagar'] == 1) {
					 				  	$checkquitar = "checked='checked'";
									  } ?>
					 			<th style="background-color: #99bfe6"><input type="checkbox" name="nopagar<?php echo $cuit?>" id="nopagar<?php echo $cuit?>" <?php echo $checkquitar ?></th>
					 			<th><b>
					 			<?php if($rowFactura['retiene'] == 1) { ?> 
			...
			<hr/>
			<input type="submit" name="submit" value="Guardar Retenciones" /><br/>
			<hr/>
			<input type="button" name="cerrar" value="Cerrar Presentacion" onclick="cerrarPresentacion(<?php echo $idPresentacion ?>)"/><br/>
			<hr/>
		</form>
	</div>
</body>
